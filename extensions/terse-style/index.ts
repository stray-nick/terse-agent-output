/**
 * terse-style — injects the terse output style into the system prompt.
 *
 * When this extension is installed, the terse rules apply to every response.
 * When it is disabled or uninstalled, the rules do not apply.
 *
 * Install (either harness):
...
export default function (pi) {
  pi.on("before_agent_start", async (event) => {
    return { systemPrompt: event.systemPrompt + TERSE_STYLE };
  });
}
